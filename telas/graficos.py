...
from dados import listar_gastos, listar_dividas, gastos_por_data
...
class GraficosFrame(ctk.CTkFrame):
    ...
    def _montar(self):
        ...
        self.frame_graficos.pack(fill="both", expand=True, padx=32, pady=(0, 20))
    ...
    def atualizar(self):
        ...
        self._top5_gastos(axes[1][1], gastos)
    ...
    def _pizza_categorias(self, ax, gastos):
        ...
        if gastos:
            cats = {}
            for g in gastos:
                cats[g["categoria"] or "Outros"] = cats.get(g["categoria"] or "Outros", 0) + (g["valor"] or 0)
            cores = ["#7c3aed","#a78bfa","#c4b5fd","#6d28d9","#4c1d95","#ddd6fe","#8b5cf6"]
            ax.pie(cats.values(), labels=cats.keys(), colors=cores[:len(cats)],
                   autopct="%1.1f%%", textprops={"color": TEXTO, "fontsize": 8},
                   wedgeprops={"linewidth": 2, "edgecolor": FUNDO})
        else:
            ax.text(0.5, 0.5, "Sem dados", ha="center", va="center",
                    color=TEXTO_MUTED, fontsize=11)
        ax.set_title("Gastos por categoria", color=VIOLETA, fontsize=11, fontweight="bold")
    ...
    def _barras_dividas(self, ax, dividas):
        ...
        if dividas:
            ...
            pendentes = [(d["valor_total"] or 0) - (d["valor_pago"] or 0) for d in dividas]
            x = range(len(credores))
            ax.bar(x, pagos,     label="Pago",     color="#7c3aed", width=0.5)
            ax.bar(x, pendentes, label="Pendente", color="#e74c3c", width=0.5, bottom=pagos)
            ...
            ax.text(0.5, 0.5, "Sem dados", ha="center", va="center",
                    color=TEXTO_MUTED, fontsize=11)
        ax.set_title("Dívidas: pago vs pendente", color=VIOLETA, fontsize=11, fontweight="bold")

    def _linha_evolucao(self, ax):
        ax.set_facecolor(CARD_BG)
        evolucao = gastos_por_data()
        if evolucao:
            datas   = list(evolucao.keys())
            valores = list(evolucao.values())
            ax.plot(datas, valores, color=VIOLETA, linewidth=2,
                    marker="o", markersize=5, markerfacecolor=ROXO_CLARO)
            ax.fill_between(range(len(datas)), valores, alpha=0.15, color=ROXO_CLARO)
            ax.set_xticks(range(len(datas)))
            ax.set_xticklabels(datas, color=TEXTO, fontsize=7, rotation=30)
            ax.tick_params(colors=TEXTO)
            ax.yaxis.set_tick_params(labelcolor=TEXTO)
            for spine in ax.spines.values():
                spine.set_edgecolor(ROXO_MEDIO)
            ...
            ax.text(0.5, 0.5, "Sem dados", ha="center", va="center",
                    color=TEXTO_MUTED, fontsize=11)
        ax.set_title("Evolução dos gastos", color=VIOLETA, fontsize=11, fontweight="bold")

    def _top5_gastos(self, ax, gastos):
        ax.set_facecolor(CARD_BG)
        if gastos:
            ...
            ax.text(0.5, 0.5, "Sem dados", ha="center", va="center",
                    color=TEXTO_MUTED, fontsize=11)
        ax.set_title("Top 5 maiores gastos", color=VIOLETA, fontsize=11, fontweight="bold")